...
exports.create = function(options, callback) {
  ...
  if (options.lifetimeInSeconds) {
    if(options.offset) {
        conditions[0].setAttribute('NotBefore', now.subtract(options.offset, 'seconds').format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'));
    } else {
  ...
  if (options.attributes) {
    ...
    Object.keys(options.attributes).forEach(function(prop) {
      ...
      attributeElement.setAttribute('AttributeName', capitalizedName);
  ...
  //Re-initializing instance of now, since the original moment is mutated when adding time earlier (in NotOnOrAfter)
  now = moment.utc();
  ...
  async.waterfall([
    function(cb) {
      if (!options.subjectConfirmationMethod && options.subjectConfirmationMethod !== 'sender-vouches')
        return cb();
...
function setNameQualifier(doc, options) {
